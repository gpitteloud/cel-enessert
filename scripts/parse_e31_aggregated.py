...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 2:
        print("Usage: parse_e31_aggregated.py <e31_file.xml>")
        sys.exit(1)
    ...
    # Parse E31 file
    parsed = parse_e31_xml(xml_file)

    if not parsed:
        print("Failed to parse E31 file")
        sys.exit(1)

    print(f"\nParsed E31 file: {xml_file.name}")
    print(f"Community: {parsed.get('community_id')} (Type: {parsed.get('community_type')})")
    print(f"Product: {parsed.get('product_code')}")
    print(f"Period: {parsed.get('start_datetime')} to {parsed.get('end_datetime')}")
    print(f"Observations: {len(parsed.get('observations', []))}")

    # Transform to data points
    data_points = transform_e31_to_datapoints(parsed)
