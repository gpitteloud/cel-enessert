...
def test_non_e31_document_returns_empty(write_xml):
    # An E66 doc type must be rejected by the E31 parser
    f = write_xml(make_e31_xml(doc_type="E66"))
    assert parse_e31_xml(f) == {}


def test_no_metering_data_returns_empty(write_xml):
...
def test_malformed_xml_returns_empty(write_xml):
    f = write_xml("<rsm:AggregatedMeteredData_13><broken>", name="bad.xml")
    # parser catches exceptions and returns {}
    assert parse_e31_xml(f) == {}
...
def test_transform_empty_input():
    assert transform_e31_to_datapoints({}) == []
    assert transform_e31_to_datapoints({"observations": []}) == []
